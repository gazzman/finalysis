...
while len(data) >= len(headers):
    curr, data = data[0:len(headers)], data[len(headers):]
    c_data, expiry, strike, p_data = curr[1:7], curr[7], curr[8], curr[9:-1]
    ...
    contract = {'ticker': cp.ticker, 'expiry': expiry_date, 'strike': strike}
